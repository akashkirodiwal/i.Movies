...
def movie_selected(request, movie_title):
    ...
    if movie.certification_label == 'A':
        messages.warning(request, f'{movie_title} is for 18+ only')
    elif movie.certification_label == 'UA':
        messages.warning(request, f'{movie_title} is for 13+ only and parental guidance is required')
    elif movie.certification_label == 'S':
        messages.warning(request, f'{movie_title} is for specialized profession only')
